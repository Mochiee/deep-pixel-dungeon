...
import com.mochi.deeppixeldungeon.R;
...
import com.mochi.deeppixeldungeon.actors.mobs.bosses.ShamanLeader;
...
public class GnollBossLevel extends Level {
    ...
    private State state;
    private ShamanLeader shamanLeader;
    ...
    private static final String SHAMANLEADER = "shamanleader";
    ...
    public void storeInBundle(Bundle bundle) {
        ...
        bundle.put(SHAMANLEADER, shamanLeader);
    ...
    public void restoreFromBundle(Bundle bundle) {
        ...
        if (state == State.START || state == State.MAZE) {
            shamanLeader = (ShamanLeader) bundle.get(SHAMANLEADER);
        } else {
            for (Mob mob : mobs) {
                if (mob instanceof ShamanLeader) {
                    shamanLeader = (ShamanLeader) mob;
                    break;
                }
            }
        }
    ...
    protected void createMobs() {
        shamanLeader = new ShamanLeader(); //We want to keep track of Terminator independently of other mobs, he's not always in the level.
    }
    ...
    public void press(int cell, Char ch) {
        ...
        if (ch == Dungeon.hero) {
            ...
            if (state == GnollBossLevel.State.START
    ...
    private void clearEntities(Room safeArea) {
        ...
        for (Mob mob : Dungeon.level.mobs.toArray(new Mob[Dungeon.level.mobs.size()])) {
            if (mob != shamanLeader && (safeArea == null || !safeArea.inside(cellToPoint(mob.pos)))) {
                mob.destroy();
                if (mob.sprite != null)
    ...
    public void progress() {
        switch (state) {
            ...
            case START:
                ...
                shamanLeader.state = shamanLeader.HUNTING;
                shamanLeader.pos = 15 + 15 * 32; //in the middle of the fight room
                GameScene.add(shamanLeader);
                shamanLeader.notice();
