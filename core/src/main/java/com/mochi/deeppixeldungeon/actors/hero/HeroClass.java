...
import com.mochi.deeppixeldungeon.items.armor.DivineArmor;
import com.mochi.deeppixeldungeon.items.artifacts.CloakOfShadows;
import com.mochi.deeppixeldungeon.items.artifacts.KingsCrown;
import com.mochi.deeppixeldungeon.items.bags.SpellBinder;
...
import com.mochi.deeppixeldungeon.items.spells.Spark;
...
import com.mochi.deeppixeldungeon.items.weapon.melee.Immortality;
...
public enum HeroClass {
	...
	private static void initMage( Hero hero ) {
		...
		if ( Badges.isUnlocked(Badges.Badge.TUTORIAL_MAGE) ){
			staff = new MagesStaff(new FlamingCloud());
		} else {
		...
		Zap zap = new Zap();
		zap.identify().collect();

		SpellBinder spellBinder = new SpellBinder();
		spellBinder.identify().collect();
	...
	private static void initRogue( Hero hero ) {
		...
		RoughKey roughKey = new RoughKey();
		roughKey.identify().collect();

		Immortality immortality = new Immortality();
		immortality.identify().collect();

		PotionOfNerveGas potionOfNerveGas = new PotionOfNerveGas();
		potionOfNerveGas.identify().collect();

		FlamingCloud flamingCloud = new FlamingCloud();
		flamingCloud.identify().collect();

        Spark spark = new Spark();
        spark.identify().collect();

		DivineArmor divineArmor = new DivineArmor();
		divineArmor.identify().collect();

		KingsCrown kingsCrown = new KingsCrown();
		kingsCrown.identify().collect();

		RatSkull ratSkull = new RatSkull();
		ratSkull.identify().collect();
