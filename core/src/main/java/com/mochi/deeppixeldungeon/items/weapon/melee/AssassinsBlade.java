...
public class AssassinsBlade extends MeleeWeapon {
	...
	public int min(int lvl) {
		return  tier + shardlvl +        //base + 1 per 2 shard levels
				lvl;            //level scaling
	...
	public int max(int lvl) {
		return  4*(tier+1) + shardlvl +   //20 base, down from 25 + 1 per 2 shard levels
	...
	public int damageRoll(Hero hero) {
		...
		if (enemy instanceof Mob && ((Mob) enemy).surprisedBy(hero)) {
			...
			int damage = imbue.damageFactor(Random.NormalIntRange((min() + max()) / 2, max()) + (Random.NormalIntRange(2*shardlvl, 4*shardlvl))); //+2 to +4 damage per shard level.
