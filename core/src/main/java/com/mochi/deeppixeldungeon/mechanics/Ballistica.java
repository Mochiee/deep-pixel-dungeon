 ...
 */
package com.mochi.deeppixeldungeon.mechanics;
...
public class Ballistica {
	...
	public static int[] trace = new int[Math.max(Dungeon.level.length(), Dungeon.level.height())];
	...
	private int build(int from, int to, boolean stopTarget, boolean stopChars, boolean stopTerrain ) {
		...
		while (Dungeon.level.insideMap(cell)) {
			...
			if (Level.losBlocking[cell]
					|| (stopChars && Actor.findChar(cell) != null)) {
				return cell;
			}
		}
	...
	public static int cast(int pos, Integer target, boolean b, boolean hitChars) {
		return pos;
	}
}
