...
import com.mochi.deeppixeldungeon.windows.WndCatalogs;
...
public class Toolbar extends Component {
	...
	protected void layout() {
		...
		int slots = DeepPixelDungeon.quickSlots();
		...
		for(int i = 0; i <= 3; i++) {
			...
			if (slots == 4 && width < 150){
				if (width < 139){
					if ((DeepPixelDungeon.flipToolbar() && i == 3) ||
							(!DeepPixelDungeon.flipToolbar() && i == 0)) {
					...
					if (i == 0 && !DeepPixelDungeon.flipToolbar() ||
						i == 3 && DeepPixelDungeon.flipToolbar()){
						btnQuick[i].border(0, 2);
						btnQuick[i].frame(106, 0, 19, 24);
					} else if (i == 0 && DeepPixelDungeon.flipToolbar() ||
							i == 3 && !DeepPixelDungeon.flipToolbar()){
		...
		float right = width;
		switch(Mode.valueOf(DeepPixelDungeon.toolbarMode())){
			case SPLIT:
				btnWait.setPos(x, y);
		...
		if (DeepPixelDungeon.flipToolbar()) {
