...
import com.mochi.deeppixeldungeon.scenes.TitleScene;
...
public class WndLangs extends Window {
	...
	public WndLangs(){
		...
		for (int i = 0; i < langs.size(); i++){
			...
			RedButton btn = new RedButton(Messages.titleCase(langs.get(i).nativeName())){
				...
				protected void onClick() {
					...
					DeepPixelDungeon.switchNoFade(TitleScene.class, new Game.SceneChangeCallback() {
						@Override
						public void beforeCreate() {
							DeepPixelDungeon.language(langs.get(langIndex));
			...
			if (DeepPixelDungeon.landscape() && i % 2 == 1){
		...
		resize(DeepPixelDungeon.landscape() ? WIDTH_L : WIDTH_P, y);
