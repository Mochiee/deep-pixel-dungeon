...
import com.mochi.deeppixeldungeon.ui.CheckBox;
import com.mochi.deeppixeldungeon.ui.OptionSlider;
		...
		public ScreenTab() {
			...
			OptionSlider scale = new OptionSlider(Messages.get(this, "scale"),
				...
				protected void onChange() {
					if (getSelectedValue() != DeepPixelDungeon.scale()) {
						DeepPixelDungeon.scale(getSelectedValue());
						DeepPixelDungeon.switchNoFade((Class<? extends PixelScene>) DeepPixelDungeon.scene().getClass(), new Game.SceneChangeCallback() {
							@Override
			...
			OptionSlider brightness = new OptionSlider(Messages.get(this, "brightness"),
				...
				@Override
				protected void onChange() {
					DeepPixelDungeon.brightness(getSelectedValue());
				}
			};
			brightness.setSelectedValue(DeepPixelDungeon.brightness());
			...
			CheckBox chkImmersive = new CheckBox( Messages.get(this, "soft_keys") ) {
				...
				protected void onClick() {
					super.onClick();
					DeepPixelDungeon.immerse(checked());
				}
			};
			chkImmersive.setRect( 0, brightness.bottom() + GAP_SML, WIDTH, BTN_HEIGHT );
			chkImmersive.checked(DeepPixelDungeon.immersed());
			...
			CheckBox chkSaver = new CheckBox( Messages.get(this, "saver") ) {
				...
				protected void onClick() {
					...
					if (checked()) {
						checked(!checked());
						DeepPixelDungeon.scene().add(new WndOptions(
							...
							protected void onSelect(int index) {
								if (index == 0) {
									checked(!checked());
									DeepPixelDungeon.powerSaver(checked());
								}
							}
						});
					} else {
			...
			chkSaver.checked(DeepPixelDungeon.powerSaver());
			if (PixelScene.maxScreenZoom >= 2) add(chkSaver);

			RedButton btnOrientation = new RedButton( DeepPixelDungeon.landscape() ?
					Messages.get(this, "portrait")
					: Messages.get(this, "landscape") ) {
				@Override
				protected void onClick() {
					DeepPixelDungeon.landscape(!DeepPixelDungeon.landscape());
				}
			};
	...
	private class UITab extends Group {
		...
		public UITab(){
			...
			RedButton btnSplit = new RedButton(Messages.get(this, "split")){
				@Override
				protected void onClick() {
					DeepPixelDungeon.toolbarMode(Toolbar.Mode.SPLIT.name());
			...
			RedButton btnGrouped = new RedButton(Messages.get(this, "group")){
				@Override
				protected void onClick() {
					DeepPixelDungeon.toolbarMode(Toolbar.Mode.GROUP.name());
					Toolbar.updateLayout();
				}
			};
			...
			RedButton btnCentered = new RedButton(Messages.get(this, "center")){
				@Override
				protected void onClick() {
					DeepPixelDungeon.toolbarMode(Toolbar.Mode.CENTER.name());
					Toolbar.updateLayout();
				}
			};
			...
			CheckBox chkFlipToolbar = new CheckBox(Messages.get(this, "flip_toolbar")){
				...
				protected void onClick() {
					super.onClick();
					DeepPixelDungeon.flipToolbar(checked());
					Toolbar.updateLayout();
				}
			};
			chkFlipToolbar.setRect(0, btnGrouped.bottom() + GAP_TINY, WIDTH, BTN_HEIGHT);
			chkFlipToolbar.checked(DeepPixelDungeon.flipToolbar());
			add(chkFlipToolbar);
			...
			final CheckBox chkFlipTags = new CheckBox(Messages.get(this, "flip_indicators")){
				...
				protected void onClick() {
					super.onClick();
					DeepPixelDungeon.flipTags(checked());
					GameScene.layoutTags();
				}
			};
			chkFlipTags.setRect(0, chkFlipToolbar.bottom() + GAP_TINY, WIDTH, BTN_HEIGHT);
			chkFlipTags.checked(DeepPixelDungeon.flipTags());
			add(chkFlipTags);

			OptionSlider slots = new OptionSlider(Messages.get(this, "quickslots"), "0", "4", 0, 4) {
				@Override
				protected void onChange() {
					DeepPixelDungeon.quickSlots(getSelectedValue());
					Toolbar.updateLayout();
				}
			};
			slots.setSelectedValue(DeepPixelDungeon.quickSlots());
			...
			CheckBox chkFont = new CheckBox(Messages.get(this, "system_font")){
				...
				protected void onClick() {
					...
					DeepPixelDungeon.switchNoFade((Class<? extends PixelScene>) DeepPixelDungeon.scene().getClass(), new Game.SceneChangeCallback() {
						@Override
						public void beforeCreate() {
							DeepPixelDungeon.classicFont(!checked());
						}
			...
			chkFont.checked(!DeepPixelDungeon.classicFont());
			add(chkFont);
		}
		...
		public AudioTab() {
			OptionSlider musicVol = new OptionSlider(Messages.get(this, "music_vol"), "0", "10", 0, 10) {
				...
				protected void onChange() {
					...
					DeepPixelDungeon.musicVol(getSelectedValue());
				}
			};
			musicVol.setSelectedValue(DeepPixelDungeon.musicVol());
			...
			CheckBox musicMute = new CheckBox(Messages.get(this, "music_mute")){
				...
				protected void onClick() {
					super.onClick();
					DeepPixelDungeon.music(!checked());
				}
			};
			musicMute.setRect(0, musicVol.bottom() + GAP_SML, WIDTH, BTN_HEIGHT);
			musicMute.checked(!DeepPixelDungeon.music());
			add(musicMute);
			...
			OptionSlider SFXVol = new OptionSlider(Messages.get(this, "sfx_vol"), "0", "10", 0, 10) {
				...
				protected void onChange() {
					...
					DeepPixelDungeon.SFXVol(getSelectedValue());
				}
			};
			SFXVol.setSelectedValue(DeepPixelDungeon.SFXVol());
			...
			CheckBox btnSound = new CheckBox( Messages.get(this, "sfx_mute") ) {
				...
				protected void onClick() {
					super.onClick();
					DeepPixelDungeon.soundFx(!checked());
					Sample.INSTANCE.play( Assets.SND_CLICK );
				}
			};
			btnSound.setRect(0, SFXVol.bottom() + GAP_SML, WIDTH, BTN_HEIGHT);
			btnSound.checked(!DeepPixelDungeon.soundFx());
			add( btnSound );
